...
int main(int argc, char *argv[])
{
    ...
    cout << " Unit test for SBSI funciton.."; cout << endl;
    cout << " tolerance: 0.005 "; cout << endl;
    SBSI sbsi;
    
    if(argv == NULL){
    cout << " Program could not execute ";
    return 1;
    }
    //
    ...
    cout << " H " << setw(5) << " WC " << setw(5) << "VTC " << setw(5) <<
            " BSA " << setw(5) << " SBSI " << setw(5) << " Function " << endl;
    ...
    for(int m = 0; m < 50; m++){
        ...
        cout << test[i*j*k*l*m + 0] << setw(5);
        cout << test[i*j*k*l*m + 1] << setw(5);
        cout << test[i*j*k*l*m + 2] << setw(5); 
        cout << test[i*j*k*l*m + 3] << setw(5); 
        cout << test[i*j*k*l*m + 4] << setw(5); 
        cout << test[i*j*k*l*m + 5] << setw(5);
        cout << endl;

        Flt tolerance = 0.005;
        Flt val = bodyShapeIndex(&sbsi);
        Flt diff = val - test[i*j*k*l*m +1];
        diff = fabs(diff);
        if(tolerance > 0.005)
        printf("  <---- Out of Tolerance function !!!!!!!!!!! \n");
        else
        printf(" \n");
